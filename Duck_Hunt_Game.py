...
font = pygame.font.Font(address+r'\assets\font\myFont.ttf', 32)
big_font = pygame.font.Font(address+r'\assets\font\myFont.ttf', 60)
WIDTH = 900
HEIGHT = 800
...
two_coords = [[], [], []]
three_coords = [[], [], [], []]
menu_img = pygame.image.load(address+r'\assets\menus\mainMenu.png')
game_over_img = pygame.image.load(address+r'\assets\menus\gameOver.png')
pause_img = pygame.image.load(address+r'\assets\menus\pause.png')
for i in range(1, 4):
    bgs.append(pygame.image.load(address+fr'\assets\bgs\{i}.png'))
    banners.append(pygame.image.load(address+fr'\assets\banners\{i}.png'))
    guns.append(pygame.transform.scale(pygame.image.load(address+fr'\assets\guns\{i}.png'), (100, 100)))
    if i < 3:
        for j in range(1, 4):
            target_images[i - 1].append(pygame.transform.scale(
                pygame.image.load(address+fr'\assets\targets\{i}\{j}.png'), (120 - (j * 18), 80 - (j * 12))))
    else:
        for j in range(1, 5):
            target_images[i - 1].append(pygame.transform.scale(
                pygame.image.load(address+fr'\assets\targets\{i}\{j}.png'), (120 - (j * 18), 80 - (j * 12))))

file = open(r'.\high_scores.txt', 'r')
...
pygame.mixer.init()
pygame.mixer.music.load(address+r'\assets\sounds\bg_music.mp3')
pygame.mixer.music.set_volume(.1)
plate_sound = pygame.mixer.Sound(address+r'\assets\sounds\Broken plates.wav')
plate_sound.set_volume(.2)
bird_sound = pygame.mixer.Sound(address+r'\assets\sounds\Drill Gear.mp3')
bird_sound.set_volume(.2)
laser_sound = pygame.mixer.Sound(address+r'\assets\sounds\Laser Gun.wav')
